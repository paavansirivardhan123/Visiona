...
class MonoDepth:
    ...
    def compute(
        ...
        for i, box in enumerate(boxes):
            ...
            # Apply 2x multiplier to correct systematic underestimation
            metric = round((self._scale / midas_val) * 2.0, 2)
    ...
    def _ensure_loaded(self):
        ...
        try:
            ...
            mt = Config.DEPTH_MODEL_ID
            print(f"  [Depth] Loading Metric Depth '{mt}' on {self._device}...")
            self._model = pipeline(task="depth-estimation", model=mt, 
                                   device=0 if torch.cuda.is_available() else -1)
            print(f"  [Depth] Depth model ready. Scale={self._scale:.1f}")
    ...
    def _run_midas(self, frame: np.ndarray) -> Optional[np.ndarray]:
        try:
            ...
            # Run inference through HF pipeline
            res = self._model(pil_img)
            # Pipeline returns a PIL Image in 'depth'
            depth_img = res["depth"]
            depth_map = np.array(depth_img, dtype=np.float32)
            # Ensure it matches original frame resolution
            if depth_map.shape[:2] != frame.shape[:2]:
                depth_map = cv2.resize(depth_map, (frame.shape[1], frame.shape[0]))
            return depth_map
        except Exception as exc:
            logger.warning(f"Depth inference log: {exc}")
            return None
