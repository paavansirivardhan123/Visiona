...
from engines.tracker import ObjectTracker
from engines.speed import SpeedEstimator
from engines.ttc import TTCCalculator
...
class VisionSystem:
    ...
    def __init__(self):
        ...
        threading.Thread(target=self.depth._mono._ensure_loaded, daemon=True).start()
    ...
    def detect(self, frame: np.ndarray, direction: str) -> List[Detection]:
        ...
        if cached and len(cached) == len(detections):
            ...
            depths = self.depth._mono._heuristic(
                yolo_boxes, [d.label for d in detections]
            )

        mode = self.depth.mode
        ...
        detections = [
            ...
            and d.distance_m <= Config.CONSIDER_MAX_M
        ]
        for d in detections:
            d.is_high_priority = d.distance_m <= Config.HIGH_PRIORITY_M
    ...
    def _maybe_start_depth(self, frame, boxes, labels):
        ...
        def _run():
            try:
                depths = self.depth._mono.compute(
                    frame, boxes, labels,
                    [None] * len(boxes)
                )
                with self._depth_lock:
