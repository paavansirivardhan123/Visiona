"""
KalmanFilter1D — scalar Kalman filter for per-track depth noise reduction.

State: [position, velocity]
Observation: [position]

Spike rejection: if |measurement - prediction| > DEPTH_SPIKE_THRESHOLD,
the measurement is treated as an outlier and the prediction is used instead.
"""
...
class KalmanFilter1D:
    ...
    def __init__(self, process_noise: float = None, measurement_noise: float = None):
        ...
        self._x: Optional[float] = None   # position estimate
        self._v: float = 0.0              # velocity estimate
        self._p00 = 1.0
        self._p01 = 0.0
        self._p10 = 0.0
        self._p11 = 1.0
    ...
    def update(self, measurement: Optional[float], dt: float = 1.0) -> float:
        """
        Update with new measurement. Returns filtered estimate.
        Pass measurement=None for prediction-only step.
        """
